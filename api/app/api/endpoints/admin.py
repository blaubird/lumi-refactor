...
logger.info(
    ...
    extra={
        "expected_token_length": (
            len(EXPECTED_ADMIN_TOKEN_ON_LOAD) if EXPECTED_ADMIN_TOKEN_ON_LOAD else 0
        )
    },
)
...
def verify_admin_token(x_admin_token: str = Header(None)):
    ...
    if x_admin_token != expected_token:
        logger.warning("Invalid admin token provided", extra={"token_match": False})
...
@router.post(
    "/tenants/",
    response_model=admin_schemas.TenantResponse,
    dependencies=[Depends(verify_admin_token)],
)
async def create_tenant(
    tenant_data: admin_schemas.TenantCreate, db: Session = Depends(get_db)
):
    """Create a new tenant."""
    ...
    if db_tenant:
        raise HTTPException(
            status_code=400,
            detail=f"Tenant with phone_id {tenant_data.phone_id} already exists.",
        )
    ...
    logger.info(
        "Tenant created",
        extra={"tenant_id": new_tenant.id, "phone_id": new_tenant.phone_id},
    )
    return new_tenant
...
@router.get(
    "/tenants/",
    response_model=admin_schemas.PaginatedResponse[admin_schemas.TenantResponse],
    dependencies=[Depends(verify_admin_token)],
)
...
async def update_tenant(
    tenant_id: str,
    tenant_update: admin_schemas.TenantUpdate,
    db: Session = Depends(get_db),
):
    ...
    logger.info(
        "Tenant updated",
        extra={"tenant_id": tenant_id, "updated_fields": list(update_data.keys())},
    )
    return db_tenant


@router.delete(
    "/tenants/{tenant_id}", status_code=204, dependencies=[Depends(verify_admin_token)]
)
...
@router.post(
    ...
    response_model=admin_schemas.FAQResponse,
    dependencies=[Depends(verify_admin_token)],
)
async def create_faq_entry(
    tenant_id: str, faq_data: admin_schemas.FAQCreate, db: Session = Depends(get_db)
):
    ...
    if embedding is None:
        logger.error(
            ...
            extra={
                "tenant_id": tenant_id,
                "question_preview": (
                    faq_data.question[:50] + "..."
                    if len(faq_data.question) > 50
                    else faq_data.question
                ),
            },
        )
        raise HTTPException(
            status_code=500, detail="Failed to generate embedding for FAQ content."
        )
...
@router.post(
    "/tenants/{tenant_id}/faq/bulk-import/",
    response_model=BulkFAQImportResponse,
    dependencies=[Depends(verify_admin_token)],
)
async def bulk_import_faq(
    tenant_id: str, import_data: BulkFAQImportRequest, db: Session = Depends(get_db)
):
    """
